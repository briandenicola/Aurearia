...
- [x] T089 [FR-026] Mark image-derived proposed fields visibly distinct from provider-cited fields in `src/web/src/components/deep-identification/DeepReportPanel.vue` and `DeepProposalEditor.vue`, using existing design tokens (`--text-muted` label, `.chip-sm`) — no new font sizes, no hardcoded colors, no emojis
- [x] T090 [FR-021, FR-026] Acceptance default in `src/web/src/components/deep-identification/DeepProposalEditor.vue`, **specified** (RD-3): **confidence-driven, not source-driven** — a field renders **accepted at confidence ≥ 0.70** and **unaccepted below 0.70**, regardless of whether its source is image-only or provider-corroborated. Consume the single named threshold constant from T120; do **not** inline `0.70`
...
- [x] T100 [F7] **Handle `build_graph()` carefully — do not just delete it.** `src/agent/app/teams/deep_identification/graph.py::build_graph` is test-only: production uses the hand-written async generator `run_deep_identification_stream`, so the topology test currently asserts on a graph users never run. The honest fix is to **first** rewrite the topology test to assert on the production generator's node sequence, **then** delete `build_graph`. Deleting it without that replacement loses coverage. **Done (Cassius)**: `tests/test_deep_identification_graph_topology.py` now drives `run_deep_identification_stream` itself (LLM/provider calls faked out) and asserts its emitted frames occur in `prepare_evidence -> router -> provider_fanout -> evaluator -> synthesizer` order; `build_graph` (and now-unused `StateGraph`/`END` imports) deleted from `graph.py` only after the new test passed
- [ ] T101 (Python portion done, Go/web remaining) [P] [F7] Remove the remaining grep-verified dead code, one commit per group, each with a grep proof in the commit body: `numista_detail()` (`src/agent/app/tools/provider_tools.py`), `RPCEnabled` (`src/api/services/settings_service.go` — **verify** it is not the flag ADR 0011 relies on to keep RPC unavailable before removing; if it is, keep it and record why), `listDeepIdentificationJobs`, `stream.reset()`, `DeepStreamTruncatedPayload`. **Done (Cassius, Python only)**: `numista_detail()` removed from `provider_tools.py` — zero call sites (grep proof in `.squad/decisions/inbox/cassius-dead-code-removal.md`). The four Go/web items (`RPCEnabled`, `listDeepIdentificationJobs`, `stream.reset()`, `DeepStreamTruncatedPayload`) are **not done** — Maximus/Aurelia own those files concurrently this batch and will sequence them separately
- [x] T102 [P] [F7] Remove or wire the unread request fields `hint_kind`, `call_budget`, `schema_version` and the unread state fields `state.errors`, `tools_base_url`, `internal_token` (`src/agent/app/models/requests.py`, `src/agent/app/teams/deep_identification/state.py`). **Caution**: `schema_version` and `call_budget` are part of the Go↔Python wire contract §2 — if they are removed from Python they must be removed from the contract and the Go mirror in the same change, or kept and documented as forward-compatibility placeholders. Prefer keeping wire fields and documenting them; remove only the genuinely internal ones. **Done (Cassius)**: `hint_kind`/`call_budget`/`schema_version` verified as genuinely unread by Python logic but present in the wire contract (§2) — kept and documented in place as forward-compatibility placeholders, no Go mirror change needed. `state.errors`/`tools_base_url`/`internal_token` verified as pure dead `TypedDict` declarations (never populated into the state dict; `run_deep_identification_stream` uses `request.tools_base_url`/`request.internal_token` directly) — removed
- [x] T103 [F5] Split `src/api/services/deep_identification_service.go` (980 lines, five responsibilities) along its existing seams — job lifecycle, worker pool, janitor/retention, SSE broker, capability/limits — into separate types in the same package with constructor injection (Principle I). **No behavior change**; the Phase 9 regression suite is the safety net. **Partial by design**: extracted the two fully clean seams (`deepIdentificationArtifactStore`, `deepIdentificationJanitor`); SSE broker/capability-limits were already separate types (`DeepIdentificationBroker`, `DeepProviderBudgetTracker`); job lifecycle + worker pool were deliberately kept together — see `.squad/decisions/inbox/maximus-service-decomposition.md`
- [x] T104 [F5] Decompose `Run` in `src/api/services/deep_identification_pipeline_runner.go` (161 lines with an 8-case inline closure) — extract the per-frame translation cases into named methods so each is unit-testable in isolation. **No behavior change**
