---
name: aurearia-software-qc-audit
description: "Audit an Aurearia major or high-risk changeset against approved scope, actual code, and exact verification evidence."
---

## Context

Run this skill for owner-authorized major/high-risk work or release readiness
under constitution section 20. Normal CI and per-change review do not replace
cross-cutting scope, contract, deployment and recovery evidence.

The audit is read-only by default. Obtain applicable authorization before tests,
builds, installations, live requests, report writes or issue creation. Use existing
exact-candidate evidence where it covers the requirement. Missing evidence is
INCOMPLETE, not passed or silently skipped. Findings are not permission to fix,
merge, deploy or clear another reviewer's block. This software audit is distinct
from the post-major-release agentic-delivery audit.

**Trigger conditions (any one suffices):**
- Material workflow or contract change spans layers (file count alone is not risk)
...
```
Use aurearia-software-qc-audit on [feature/PR/branch description].
...
3. Confirm the spec, PRD section, or design doc that authorized this work.
   `[REPO-HOOK]` Check constitution section 0: Constitution → PRD → applicable accepted ADRs → active spec → plan → tasks → backlog → active decisions → judgment.
...
- **Layering violations:** Check the actual project import matrix. Aurearia thin
  handlers may call services or repositories; services stay HTTP-agnostic and
  repositories own queries. Verify real architecture test selectors before using
  them, and apply `task check:go` when authorized and applicable.
...
- **Supply chain:** Review manifest constraints and exact lock resolution.
  Executable tool/action/image pins must be reviewed; do not treat a floating
  `latest` tool install as a reproducible dependency.
...
- **Schema migration safety:** Prove representative existing-data upgrades,
  preservation and recovery. SQLite table rebuilds require explicit safety
  evidence, not a blanket assumption of safety or failure. Renames, drops,
  constraints and defaults must respect the approved data contract.
- **Pydantic / TypeScript interface alignment:** Python Pydantic models, Go response structs, and TypeScript interfaces that represent the same resource must agree on field names and optionality.
  `[REPO-HOOK]` Use `task check:web` where applicable; `--noEmit` is not strict-build parity.
...
- **Coverage of new paths:** Map changed acceptance criteria and material failure
  paths to actual tests. Respect section 21's new-service-method unit-test rule;
  do not invent a requirement for a separate unit test for every trivial helper.
...
- **New dependencies:** For every entry added to `go.mod`, `package.json`, `pyproject.toml`, or equivalent:
  ...
  - Are manifest constraints and exact resolved lock versions reviewed?
...
[PASS / BLOCK / INCOMPLETE; exact candidate identity, covered domains, N/A
reasons, unavailable evidence, unexecuted checks and remaining approval boundaries]
```

**Severity rules:**
- **Blocker:** Evidence-backed exploitable path, data-loss risk, unmet binding
  acceptance requirement, broken contract, applicable gate failure, or confirmed
  constitutional violation. Cite the actual requirement and observed mechanism.
- **Follow-Up:** Non-binding improvements in tests, documentation or observability.
  Do not downgrade a binding requirement because it looks stylistic.
- **Incomplete evidence:** Missing environment, unread scope or unverified claims.
  State what is missing without fabricating a defect or reporting PASS.
