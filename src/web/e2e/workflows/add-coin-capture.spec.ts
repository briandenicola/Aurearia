...
for (const mode of ['pwa', 'desktop'] as const) {
  test(`${mode} Add Coin shares Identify capture layout and saves reviewed evidence`, async ({ page }, testInfo) => {
    ...
    if (mode === 'desktop') {
      ...
      const titleBounds = await header.getByRole('heading', { name: 'Add Coin', exact: true }).boundingBox()
      const pillsBounds = await header.getByRole('button', { name: 'AI Assist Mode', exact: true }).boundingBox()
      const headerBounds = await header.boundingBox()
      expect(titleBounds).not.toBeNull()
      expect(pillsBounds).not.toBeNull()
      expect(headerBounds).not.toBeNull()
      if (!titleBounds || !pillsBounds || !headerBounds) throw new Error('Add Coin header is not visible')
      expect(Math.abs(titleBounds.y + titleBounds.height / 2 - pillsBounds.y - pillsBounds.height / 2)).toBeLessThanOrEqual(1)
      expect(Math.abs(pillsBounds.x + pillsBounds.width - headerBounds.x - headerBounds.width)).toBeLessThanOrEqual(1)
