        ...
        <div v-if="results" class="flex flex-col gap-6">
          ...
          <form v-if="ngcCertNumber" class="card" @submit.prevent="handleSaveAsDraft">
            ...
            <div v-if="aiObservations" class="mt-2">
              ...
              <div class="rounded-sm border border-border-subtle bg-input p-3 text-body leading-6 text-text-secondary [&_ol]:mb-3 [&_p]:mb-3 [&_p:last-child]:mb-0 [&_strong]:font-semibold [&_strong]:text-gold [&_ul]:mb-3" v-html="renderedAiObservations"></div>
            </div>
          </form>
