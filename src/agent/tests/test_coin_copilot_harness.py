...
async def test_multi_tool_completion_is_sequential_and_typed():
    model = _SequenceModel([
        AIMessage(content="", tool_calls=[{
            "name": "collection_summary", "args": {}, "id": "call_1", "type": "tool_call"
        }]),
        AIMessage(content="", tool_calls=[{
            "name": "gap_analysis", "args": {}, "id": "call_2", "type": "tool_call"
        }]),
...
async def test_duplicate_and_malformed_tool_calls_fail_closed():
    ...
    malformed_model = _SequenceModel([
        AIMessage(content="", invalid_tool_calls=[{
            "name": "get_coin", "args": "{bad", "id": "bad", "error": "invalid"
        }])
    ])
...
async def test_tool_and_iteration_budgets_stop_before_next_operation():
    ...
    second = AIMessage(content="", tool_calls=[{
        "name": "get_coin", "args": {"coin_id": 1}, "id": "call_2", "type": "tool_call"
    }])
    summary = {"summary": {
        "totalCoins": 0,
        "totalWishlist": 0,
