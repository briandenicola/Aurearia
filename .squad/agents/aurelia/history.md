...
- **CoinDetailPage Purchase Row Integration**: Moved "Purchased {date} from {store}" line from standalone `.purchase-meta` section (above Details heading) into the Details metadata table as the final row. Added optional `fullWidth: boolean` property to `CoinDetailMetadataRow` interface (types/index.ts). Updated `CoinDetailMetadataTable.vue` to handle full-width rows differently: `v-if="!row.fullWidth"` on label, `:class="{ 'full-width': row.fullWidth }"` on row wrapper, `grid-column: 1 / -1` CSS for `.full-width` class. Updated `useCoinDetailMetadataRows` composable to add purchase row at the end when `coin.purchaseDate || coin.purchaseLocation` is present. Removed unused `SafeExternalLink` component import and `safeReferenceUrl` computed from `CoinDetailPage.vue`. Full-width row uses `font-style: italic; color: var(--text-secondary)` to visually match the original purchase-meta styling. Type-check and lint pass clean.

- **Purchase Row Refinement — Store-Only with Optional Link (2026-06-01)**: Refined purchase location row to display ONLY the store name (no date, no "Purchased from" prefix). Date is already shown in its own "Purchase Date" row. Extended `CoinDetailMetadataRow` interface with optional `url?: string | null` field (types/index.ts). Updated `useCoinDetailMetadataRows` composable to: (a) only render row when `coin.purchaseLocation` is present, (b) pass `sanitizeExternalUrl(coin.referenceUrl)` as the `url` field. Updated `CoinDetailMetadataTable.vue` to conditionally render `SafeExternalLink` component when `row.url` is present (clickable, opens in new tab, `rel="noopener"`, sanitized), or plain text when no url. Store link styled with `--accent-gold` → `--accent-bronze` on hover, using design tokens. Reused existing `SafeExternalLink` component + `sanitizeExternalUrl` helper from `@/composables/useSafeExternalLink` — no duplication. Field used: `coin.referenceUrl` (same as prior standalone line). Type-check and lint pass clean. Decision #16 merged to decisions.md.

- **Store Prefix Label (2026-06-01)**: Added "Store: " prefix to the purchase location full-width row. Updated `CoinDetailMetadataTable.vue` to conditionally render prefix (`<span class="store-prefix">Store: </span>`) only for the `purchaseLocation` row (checked via `row.key === 'purchaseLocation'`). Prefix styled with `font-style: italic; color: var(--text-muted)` to match full-width row styling. Store name follows prefix as either plain text or `SafeExternalLink` depending on `row.url` presence. When linked, only the store name is clickable (gold accent, hover bronze transition) — prefix remains plain text. Result: row displays as "Store: {store name}" with the store name linked when `coin.referenceUrl` is present. No composable changes needed (row already has `key: 'purchaseLocation'`). Type-check and lint pass clean with no new warnings.
