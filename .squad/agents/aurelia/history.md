...
- **2026-08-17 — Feature 351 T016 (quick-lookup outcome surfacing):** Added an optional `quickLookupOutcome` field ('ok' | 'no_data' | 'unavailable') to the `DeepReport` type in `types/index.ts` and rendered it in `DeepReportPanel.vue`: 'ok' is invisible (no extra DOM), 'no_data' is a quiet muted-text note, 'unavailable' gets a bordered warning-token box (reusing the text-warning/color-mix pattern already used in SettingsAccountSection.vue) stating plainly that the lookup did not finish and results may be incomplete -- the exact distinction that would have saved an hour of confusion on the Maximinus I NGC-slab run. Missing-key (pre-change reports) renders identically to today, verified by a dedicated test. Added 4 new component tests (ok / no_data / unavailable / key-absent) to the existing DeepReportPanel.test.ts suite. Investigated Admin Settings exposure per the task's side request: AdminSystemSection.vue already exposes DeepIdentificationWorkerCount, MaxActivePerUser, QueueDepth, HardTimeoutSeconds, EventRetentionHours, ResultRetentionDays, MaxProviders, NumistaCallBudget, OCREEnabled, and OCRECallBudget via one declarative numeric-fields list plus matching props/refs/watchers. DeepIdentificationQuickLookupTimeoutSeconds (new sibling setting in this same feature) is not yet in that list -- exposing it would be a small, well-scoped follow-up (one prop + default + emit type + ref + watch + one list entry, then thread the value through the parent settings page and useAdminConfig.ts defaults) but was reported only, not built, per task scope. Gates run and passed: vue-tsc --build clean; full vitest run 130 files / 817 tests green.
...
- **2026-07-02 — #374 Coin Grading UI Implementation & Approval (Complete):** Added coin grading UI as an AI analysis sub-action within `CoinAIAnalysis.vue`. Feature requires both obverse and reverse images (disabled when either missing), uses existing AI job polling pattern, displays `result.gradingReport` as sanitized Markdown with permanent limitation copy ("AI estimate does not update Coin.grade automatically"). Added client support via `gradeCoin()` call. All targeted Vitest tests pass; `npm run type-check` passes; no regressions to existing analysis workflow. Brutus approved with final QA sign-off. Outcome: APPROVED. Feature ready for merge. Session log: .squad/log/2026-07-02T10-55-14-coin-grading-workflow.md.

- **2026-07-02 — Issue #371 Price Alerts and Bid Reminders UI (Complete):** Added typed frontend contracts for auction `PriceAlert`, `BidReminder`, and `AuctionAlertReminderRun`. Auctions now loads `/alerts` and `/reminders`, shows compact alert/reminder chips on lot cards, and lets users create/delete price alerts plus bid reminders from `AuctionLotDetailModal` while displaying triggered/notified state. Admin Schedules gained an "Auction Price Alerts and Bid Reminders" settings panel plus Run Now/history wired to Cassius's `/admin/auction-alert-runs` and `/admin/auction-alerts/run` backend contract. Validation: `npm.cmd run test -- AuctionLotDetailModal AdminSchedulesSection AuctionsPage`, `npm.cmd run type-check`, and `npm.cmd run build` all passed from `src/web/`.
...
- **2026-08-11 (Feature 341 Phase 4 photo/draft workflow):** Identify Coin now consumes additive `proposedNumistaQuery`/`numistaEvidence` fields, waits for an explicit collector search, preserves NGC-first behavior, and saves only the selected candidate into Quick Capture multipart fields. Draft editing seeds the shared lookup panel from `selectedNumistaReference` and uses a tri-state mutation (`unchanged` omits fields, `replace` sends the ID/URL pair, `clear` sends `clearSelectedNumista=true`) so unrelated edits and validation failures cannot silently remove or replace a reference. Shared panel action buttons require `type="button"` because the component is reused inside the draft edit form.
- **2026-08-11 (Feature 341 promotion safety):** Quick Capture promotion must be disabled while a Numista selection replacement/removal is unsaved. Promotion copies the persisted draft relation, so allowing promotion with pending frontend selection state could copy the previous reference; the readiness panel now requires Save Changes first.
- **2026-08-11 (Feature 341 Phase 5 status UX):** The shared Numista panel now models `idle` and `loading` alongside all six domain outcomes, announces every state with visible text, and focuses the terminal status region after a lookup. Query edits become locally dirty on first input so later parent evidence updates cannot overwrite them; selected candidates remain retained across empty/error/retry transitions. Only admins receive the `/admin?tab=system` configuration link and API-key instructions, while ordinary users receive non-privileged guidance. Retry-After and cache freshness text render only from supplied contract metadata, with no quota estimate.
- **2026-08-11 (Feature 341 User Story 6 T092–T095):** Saved-coin Numista lookup now lives as a keyboard-accessible inline disclosure in Catalog References, receives the complete available coin evidence, and collapses only after persistence plus a refreshed matching reference. Actions retains one compact link to the existing detail anchor. Identify Coin preserves non-NGC lookup, adds an explicit no-eager-request NGC override, and uses `Analyze Photos` / `Save as Draft`. Draft cards render retained wrapping `Numista #<identifier>` chips without another request. Focused/related Vitest 47/47, full Vitest, type-check, explicit `vue-tsc --build`, production build, design-token tests, lint (0 errors), and diff check passed.
- **2026-08-11 (Feature 341 Phase 6 T062–T063):** Admin System now exposes the six approved bounded Numista cache, enrichment, result, and timeout settings and persists their validated string values through the existing settings workflow. A redacted admin-only health view shows configuration validity, complete status counts, request totals, p50/p95 latency, cache hit/refresh rate, enrichment outcomes, and observed quota timing without rendering response extras, raw errors, queries, evidence, credentials, or estimated remaining quota. T057, related admin/design tests, full Vitest, lint, type-check, production build, and diff check passed.
...
- **2026-08-12 — Feature 342 frontend integration (T014–T020):** Added strict proposal/source/attempt DTOs and the authenticated proposal API wrapper; direct and Quick Capture draft defaults now load server-owned proposals from evidence only. `NumistaLookupPanel` tracks generated, sticky user-edited, manual, and server-downgraded states, preserves exact editable input, and discloses relaxed effective queries separately while retaining selection, enrichment, focus, accessibility, NGC override, and no-eager-search behavior. Removed duplicate TypeScript query assembly. Focused Feature 342 tests (105), full Vitest, changed-file lint, strict type-check/build, design tokens, and Feature 341 regressions passed; full lint retained 164 pre-existing warnings and zero errors.

## Learnings

- **2026-08-17 — Feature 351 Phase 12a (T085–T088):** Fixed the lying "Reconnecting…" connection badge on `DeepAnalysisProgressTimeline.vue`/`useDeepIdentificationStream.ts` by implementing a **real** reconnect (option a), not the fallback rename: `DeepAnalysisPage.vue` now watches `stream.connected` for an unexpected `true→false` transition (never on an intentional teardown, guarded by a `suppressAutoReconnect` flag) and re-calls `stream.connect(jobId, { since: stream.lastSeq.value })` with a capped, backed-off retry schedule (1s/2s/5s/10s, 4 attempts), reusing the exact resume mechanism from `contracts/sse-events.md` §3 that already powers resume-on-mount. The Timeline gained a `reconnecting` prop so "Reconnecting…" is asserted true only while a reconnect is actually scheduled/in-flight, plus a "Disconnected" state with an explicit Retry button for when auto-retry gives up or the initial connect never even succeeded. Composable itself (`useDeepIdentificationStream.ts`) was deliberately left untouched — all timer/backoff logic lives in the page component so the composable's existing unit tests (which use bodies that close without an `end` frame) stay unaffected and no stray real-time `setTimeout`s leak across test files.
- Extracted `useDeepAnalysisLauncher` composable (capability gate + start-job + navigate-to-`/deep-analysis/:id`) consumed identically by `CoinActionsPanel.vue` and `CoinLookupPage.vue` — the previously verbatim-duplicated launch sequence now exists in exactly one place (grep-verified: only one `deepIdentification.start(` call site in the whole `src/web` tree, inside the composable).
- **`DeepIdentificationMaxActivePerUser` is NOT exposed to normal users on any existing endpoint** — `GET /deep-identification/capability` returns only `{enabled, providers}`; the numeric limit lives in admin-only settings (`src/api/services/settings_service.go`). The one real, already-existing (previously unused) signal on the client is `listDeepIdentificationJobs({ activeOnly: true })`, which reports the current user's own active-job count. `useDeepAnalysisLauncher` uses "count > 0" as the disable trigger for `DeepAnalysisEntryButton.disabled` — accurate for the documented default limit of 1, conservative (never worse than the previous unwired-prop bug) for admin-configured higher limits. Do not invent a `maxActivePerUser` field on the client without a real backend contract change.
- **2026-08-17 — Feature 351 FR-040 activity timeline (frontend half, backend blocked):** Built `DeepAnalysisActivityTimeline.vue`, a new collapsible (`<details>`, auto-collapses once on first terminal transition, never fights a manual toggle) activity log embedded inside the existing `DeepAnalysisProgressTimeline.vue` (which keeps owning the connection badge/retry/cancel controls). It derives structured steps from the raw `DeepStreamEvent[]` stream client-side — no backend change needed to ship the visual layer: known event types (`job_accepted`, `router_selected`, `evaluation`, `synthesis_started`, `terminal`) get curated labels; `progress` events use a small `knownPhaseLabels` lookup with a generic `titleCase(phase)` fallback for **any** unrecognized phase; any totally unrecognized event `type` also falls back to `titleCase(type)` + first string payload field — nothing is ever silently dropped, per FR-040's "design for unknown phases" requirement. `provider_started`/`provider_result` events are grouped into a single "Provider fan-out" step containing one badge row per provider (reusing the `DeepProviderStatus` vocabulary/labels already defined for `DeepProviderCoverageList.vue`), so `no_match`/`skipped`/`not_automated`/`unavailable` render as a distinct "No result" state (icon + chip + wording) that can never be mistaken for the gold "Done"/"Contributed" success state — this directly targets Brian's "silent nothing" complaint. Elapsed-time deltas are computed from consecutive event `ts` values (`+Ns`/`+Nm Ns` format) when parseable, and skipped silently when not (never crashes on a bad timestamp). The exact new backend `progress` phase/message shapes this UI is designed to consume (to hand to the backend agent) are: `vision_completed` (populated-field count + confidence bucket, e.g. "Vision analysis produced 6 populated fields (high confidence)."), `provider_query_dispatched:<provider>` (the query terms actually sent), `provider_settled:<provider>` (mirrors `provider_result` payload — prefer emitting a real `provider_result` event instead of a progress phase for this), and `synthesis_started` detail (candidate/claim count feeding synthesis). Verified: `npm.cmd run type-check` exit 0; full `npx.cmd vitest run` 131 files / 830 tests passed (up from a stated baseline of 130/821 — confirmed the delta is exactly the new `DeepAnalysisActivityTimeline.test.ts` file's 9 tests).
- **2026-08-17 — Feature 351 Phase 12b (T089–T092, T120): hypothesis panel + confidence-driven acceptance.** Traced the exact wire shape before writing UI: `DeepSynthesis.image_hypothesis` is passed from Python to the browser **verbatim** (`src/api/handlers/deep_identification.go:97` returns `json.RawMessage(job.ReportJSON)`; `deep_identification_pipeline_runner.go:218` stores `payload.Report` raw), so the real JSON key genuinely is snake_case `image_hypothesis`, not `imageHypothesis` — added it to `DeepReport` (`types/index.ts`) with an explicit comment flagging the deliberate exception to this interface's otherwise-camelCase convention, backed by `contracts/vision-hypothesis.md` §4/§7 and the literal key name used throughout `tasks.md` T091/T094/T071. Built the collapsible (`<details>`, default-closed — no `open` attr) "What the images alone said" section in `DeepReportPanel.vue`: hidden entirely (no empty shell) when `report.image_hypothesis` is absent; when `legible: false` shows dedicated wording ("not legible enough... different from the analysis not running at all") distinct from the separate "legible but nothing found" empty-fields case, so the exact ambiguity Brian's Maximinus run exposed can no longer recur. **The acceptance-threshold discovery**: confirmed via `deep_identification_pipeline_runner.go:949`/`993` (`Accepted: nil` always at proposal-build time) and `deep_identification_proposal.go:231` (`Apply()` only ever writes fields with **persisted** `Accepted == true`) that RD-3's confidence-driven default cannot be purely a render-time cosmetic — a field only reads as "accepted" today if a PATCH literally set `accepted:true` server-side, so `DeepAnalysisPage.vue::onApplyProposal` now walks the proposal and PATCHes any still-null, confidence-qualifying field to `accepted:true` immediately before calling Apply (sequential `await`s, not fire-and-forget, to avoid a race against the PATCH landing). Extracted the single named constant + effective-acceptance logic to `src/web/src/utils/deepProposalAcceptance.ts` (`DEEP_PROPOSAL_ACCEPTANCE_THRESHOLD = 0.7`), consumed by `DeepProposalEditor.vue` (button render state + `confirmDisabled`) and `DeepAnalysisPage.vue` (pre-apply finalize) — no bare `0.70` literal anywhere (T120). **Checked whether a matching Python/Go-side constant exists to keep in sync**: it does not — Go always persists `Accepted: nil` regardless of confidence, so there is no synthesis-side default to align with; nothing to route to a backend agent on this point. Image-derived marking (T089) reads `evidence.length === 0` (an *empty array*, not *absent*) per `data-model.md` §6 — deliberately does **not** mark the pre-existing narrative-fallback `notes` field (which has no `evidence` key at all) as image-derived, avoiding a false-positive label on unrelated legacy behavior. Extracted `formatFieldName` (underscore + camelCase → Title Case) into `src/web/src/utils/format.ts`, replacing the duplicated inline regex in `DeepProposalEditor.vue` and reusing it in the new hypothesis panel. Extended `DeepClaim.citation` to optional (an image-sourced disagreement claim has no citation per contract §3) and render a `.chip-sm` "From images" marker in place of the source link when absent. Verified: `npm.cmd run type-check` exit 0; full `npx.cmd vitest run` 131 files / 842 tests passed (up from the 131/830 baseline — the +12 delta is exactly the new assertions added to `DeepProposalEditor.test.ts` and `DeepReportPanel.test.ts`).
- **2026-08-17 — `job_at_capacity` (HTTP 409) UX for Deep Analysis submit, paired with Cassius's backend fix.** Backend was changing the wrong-coin-report bug (a genuinely different in-flight job at `MaxActivePerUser` returning a fully-populated 200 for the *previous* coin, `reused:true`) to a proper `409 {error, code:"job_at_capacity"}` — idempotent duplicate submissions (same images resubmitted) are unaffected and still return 200/`reused:true`. Read the existing error path first per the task brief: `deepIdentification.error` was already wired end-to-end into `DeepAnalysisStartPanel`'s `submitError` prop via `useDeepAnalysisLauncher.submitDeepAnalysis`, and `starting`/spinner already reset in a `finally` — so no parallel error mechanism was needed, only a code-aware branch. Added `getApiErrorCode(error)` to `api/client.ts` (sibling to `getApiErrorMessage`, same `response.data` extraction, but reads `.code` not `.error`/`.message`/`.detail`) and a new `errorCode` ref + fallback text on `useDeepIdentification.start()`. For the "point the user at it" requirement, extended `useDeepAnalysisLauncher` (already probes `listDeepIdentificationJobs({activeOnly:true})` for the entry-button disable state) to also remember the active job's id (`activeJobId`) and, only on a `job_at_capacity` rejection, refresh that probe and expose `capacityConflictJobId` — `DeepAnalysisStartPanel` renders a `.btn.btn-secondary.btn-sm` `RouterLink` to `/deep-analysis/:id` next to the error text when set, which is the *same* job page that already has the Cancel control (`DeepAnalysisProgressTimeline`), so this is a navigation link, not a duplicate inline cancel button — kept it small per Principle IV rather than wiring a second cancel affordance into the modal. Both call sites (`CoinActionsPanel.vue`, `CoinLookupPage.vue`) needed the one new prop wired since they share the panel but not a shared modal component. Test gotcha: mocking `listDeepIdentificationJobs` to return an active job for the *whole* test made the entry button start disabled and the modal never open — needed `mockResolvedValueOnce` (empty on mount) then the real active-job payload for the post-409 refresh call. Verified: `npm.cmd run type-check` exit 0; full `npx.cmd vitest run` 131 files / 848 tests passed (up from 131/843 — the +5 is `useDeepIdentification.test.ts` (+2: capacity error + code, message fallback), `DeepAnalysisStartPanel.test.ts` (+2: conflict link renders/doesn't render), `CoinLookupPage.test.ts` (+1: full page-level assertion that the actionable message, link, and non-stuck-spinner recoverable state all render together).
- **2026-08-18 — Phase 12b follow-up: the pre-Apply finalize loop was a silent-failure trap, fixed by batching.** Code review (Brian) caught that `useDeepIdentification.ts::updateProposalField` catches its own error, sets `error.value`, and returns `null` **without throwing** — so the original `onApplyProposal` loop (`await deep.updateProposalField(...)` per field, ignoring the return) would swallow a failed PATCH and fall straight through to `applyProposal`, applying a partial proposal while reporting success. This is exactly the failure class the whole 351 effort exists to eliminate, reintroduced at the very last write. Fixed by (1) checking the Go side first — `deep_identification_proposal.go::UpdateProposal` (lines 176-208) validates every field name in an incoming multi-key `fields` map *before* applying any edit, then commits the whole document in one atomic save, so a single batched PATCH is both safe and strictly better than N sequential ones (fixes atomicity too: no more half-written proposal if the 3rd of 14 PATCHes fails); (2) adding `updateProposalFields(jobId, edits: Record<string, DeepProposalFieldEdit>)` to `useDeepIdentification.ts`, mirroring the existing single-field function's optimistic-update/error-swallow contract (still returns `null` on failure — callers MUST check); (3) rewriting `onApplyProposal` to build one `edits` map (still filtered on `entry.accepted === null` to preserve the explicit-user-rejection guard), send it in one `updateProposalFields` call, and **return before calling `applyProposal`** if that call returns `null`, surfacing `deep.error.value` (or a fallback message) in `applyError` so the failure is visible and the proposal stays unapplied for retry. Added a regression test (`DeepAnalysisPage.test.ts`) that mocks `patchDeepIdentificationProposal` to reject, then asserts `applyDeepIdentificationProposal` is never called, no navigation happens, and the rejection message renders — a guard with no test enforcing it is not a guard. **Lesson for future work in this feature**: any composable function that returns `null`/swallows its own error instead of throwing puts the burden of checking on every call site forever; when adding a new call site, grep the function body first, don't assume `await` = "will throw on failure." Verified: `npm.cmd run type-check` exit 0; full `npx.cmd vitest run` 131 files / 843 tests passed (up from 131/842 — the +1 is the new pre-Apply-failure guard test).
