...
- **Watch Bid Digest** — A scheduled digest of every active watched or bidding lot. Each lot shows its title and lot number, the sale, and its current high bid compared with the bid the previous digest reported (`up from 75.00`, `down from 95.00`, `no change`); a lot reported for the first time shows its bid alone. The comparison baseline advances only when a digest is delivered, so an undelivered or length-trimmed lot keeps its change for the next digest.
